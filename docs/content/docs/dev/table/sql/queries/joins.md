...
-- Returns a new row for each element in the array assuming a Orders table with an array column `product_names`
...
The order of the elements for arrays and multisets is guaranteed. Since maps are unordered, the order of the elements is not guaranteed.
